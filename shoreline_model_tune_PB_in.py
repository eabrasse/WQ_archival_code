...
def solve(nt,ny,Vmod,PB_in,kt,kd):
...
if rayleigh:
    ...
    # kd = 7.5e-6 # EAB 1/18/22 - testing to see if concentrations match better
    # kd = 2e-6 # EAB 4/18/22 - testing to see if concentrations match better
    # kd = 4.25e-7 # EAB 4/18/22 - testing to see if concentrations match better
    # PB_in0 = 0.007
    PB_in0 = 0.01
else:
    V = Isobath5m['V'][:]
    # kd = 1.404e-5 # EAB 1/18/22 - testing to see if concentrations match better
    # kd = 1e-5 # EAB 4/18/22 - testing to see if concentrations match better
    PB_in0 = 0.01
...
for PB_in in PB_in_array:
    ...
    c,v=solve(nt,ny,Vmod,PB_in,kt,kd)
    ...
    for j in range(ny-y_ind):
        wss_alongshore[j] = wqfun.willmott(c[:,y_ind+j],Ddye['dye_01'][:,y_ind+j])
...
while True:
    ...
    tic = time.perf_counter()
    c,v=solve(nt,ny,Vmod,PB_in_guess,kt,kd)
    toc = time.perf_counter()
    model_time = f"shoreline model took {toc-tic:0.4f} seconds"
    print(model_time)
...
if yn=='x':
    ...
    if rayleigh:
        model_type_string = 'rayleigh'
    else:
        model_type_string = 'sawc'
    model_name = 'autotuned_'+model_type_string+f'_kd{kd:.4E}_uniformv_5miso_PB_in{PB_in:0.3f}'
